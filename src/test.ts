type db = {
    ...
    methods: {
        method1: {
            args: [str:string, nr:number],
            returns : string
        }
        method2: {
            args: [str2:string, nr1:number],
            returns : number
        }
    }
}
type Methods = {
    [methodName in keyof db['methods']]: (...args: db['methods'][methodName]['args']) => db['methods'][methodName]['returns']
}



let handler: ProxyHandler<Methods> = {
    get<T extends keyof Methods>(target: {}, p: T, receiver: any): any {
        console.log(`get called with ${String(p)}`);
        return function (): ReturnType<Methods[T]> {
            return `get called with ${String(p)}` as ReturnType<Methods[T]>; 
        }
    }
}
